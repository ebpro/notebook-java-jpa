...
public class DatabaseManager {
    ...
    static {
        ...
        for (String[] config : configVariables) {
            String property = System.getProperty("jakarta.persistence.jdbc.url");
            if (!(property == null || property.isBlank())) {
                log.info("Override JPA config {} with system property {}", config[0], config[1]);
                configOverrides.put(config[0], System.getProperty(config[1]));
            } else if (env.containsKey("DATASOURCE_URL")) {
                log.info("Override JPA config {} with env variable {}", config[0], config[2]);
                configOverrides.put(config[0], config[2]);
            }
        }
